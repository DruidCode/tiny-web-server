...
{
    int listenfd, connfd, port, clientlen, epollfd, nfds, n;
	...
	if (epoll_ctl(epollfd, EPOLL_CTL_ADD, listenfd, &ev) == -1) {
		fprintf(stderr, "error: epoll add error");
		exit(1);
	}
	...
	for(;;) {
		...
		if ( nfds == -1 ) {
			...
			fprintf(stderr, "error: epoll wait error %d", errno);
			exit(1);
		}
		...
		for ( n=0; n<nfds; ++n) {
			...
			if (events[n].data.fd == listenfd) { //new connect
			...
			} else if (events[n].events&EPOLLIN) { //TODO
				doit( events[n].data.fd );
				fprintf(stdout, "info: it is over");
			} else if (events[n].events & EPOLLOUT) { //TODO
			} else {
			}
		}
	}
